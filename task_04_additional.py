...
class Median:
    ...
    def max_heap_insert(self, key):
        ...
        self.heap_increase_key(len(self.high_array)-1, key)
    ...
    def build_max_heap(self):
        for i in range(len(self.high_array) // 2, -1, -1):
            self.max_heapify(i)
    ...
    def max_heapify(self, i):
        ...
        if l < len(self.high_array) and self.high_array[l] > self.high_array[i]:
            ...
            largest = i
        if r < len(self.high_array) and self.high_array[r] > self.high_array[largest]:
            largest = r
        if largest != i:
            self.high_array[i], self.high_array[largest] = self.high_array[largest], self.high_array[i]
            self.max_heapify(largest)


    def heap_extract_max(self):
        maximum = self.high_array[0]
    ...
    def min_heapify(self, i):
        ...
        if l < len(self.low_array) and self.low_array[l] < self.low_array[i]:
            ...
            smallest = i
        if r < len(self.low_array) and self.low_array[r] < self.low_array[smallest]:
            smallest = r
        if smallest != i:
            self.low_array[i], self.low_array[smallest] = self.low_array[smallest], self.low_array[i]
    ...
    def min_heap_insert(self, key):
        ...
        self.heap_decrease_key(len(self.low_array)-1, key)
    ...
    def add_element(self, value):
        ...
        if len(self.high_array) - len(self.low_array) > 1:
            self.min_heap_insert(self.heap_extract_max())
            self.build_max_heap()
            self.build_min_heap()
        elif len(self.low_array) - len(self.high_array)>1:
            self.max_heap_insert(self.heap_extract_min())
            self.build_max_heap()
            self.build_min_heap()

    def get_median(self):
        if len(self.high_array) - len(self.low_array) == 0:
            return (self.high_array[0], self.low_array[0])
        else:
            if len(self.high_array) > len(self.low_array):
